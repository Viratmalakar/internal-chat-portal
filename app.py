...
def create_admin():
    ...
    if not cur.fetchone():
        ...
        VALUES('admin','System Admin','Bfil@123','admin')
        """)
        con.commit()
    con.close()
...
def compose():
    if request.method=="POST":
        ...
        cur = con.cursor()
        cur.execute("""
        INSERT INTO messages(sender,receiver,cc,subject,message,file,time)
        VALUES(?,?,?,?,?,?,?)
        """,(session["user"],to,cc,subject,msg,filename,str(datetime.now())))
        con.commit()
        con.close()
